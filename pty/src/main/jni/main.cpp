...
static jboolean JNICALL m_setWorkDir(JNIEnv *const env, const jobject jthis, const jstring path) {
    ...
    if (m_workDir != nullptr) {
        free(m_workDir);
        m_workDir = nullptr;
    }
    m_workDir = (char *) malloc(len + 1);
    if (m_workDir == nullptr) {
        return false;
    }
    strncpy(m_workDir, workPath, len);
    m_workDir[len] = '\0';
    env->ReleaseStringUTFChars(path, workPath);
    return true;
}
...
static const JNINativeMethod methodTable[] = {
        {"execve",                 "(Ljava/lang/String;[Ljava/lang/String;[Ljava/lang/String;)L" CLASS_NAME ";",
                                                            (void *) m_execve},
        {"setWorkDir",             "(Ljava/lang/String;)Z", (void *) m_setWorkDir},

package com.skyworthdigital.speedtest.core.serverSelector;
...
public abstract class ServerSelector {
    ...
    private com.skyworthdigital.speedtest.core.serverSelector.TestPoint selectedTestPoint = null;
    ...
    public void addTestPoint(com.skyworthdigital.speedtest.core.serverSelector.TestPoint t) {
        if (state != NOT_STARTED) {
            throw new IllegalStateException(
                    "Cannot add test points at this time");
        }
        if (t == null) return;
        servers.add(t);
    }
    ...
    private void next() {
        ...
        synchronized (mutex) {
            if (tpPointer >= servers.size()) {
                if (activeStreams <= 0) {
                    ...
                    for (TestPoint t : servers) {
                        ...
                        if (selectedTestPoint == null || t.ping < selectedTestPoint.ping) {
                            selectedTestPoint = t;
                        }
                    }
            ...
            PingStream ps = new PingStream(tp.getServer(), tp.getPingURL(), PINGS,
                    SpeedTestConfig.ONERROR_FAIL, timeout, timeout, -1, -1, null) {
                @Override

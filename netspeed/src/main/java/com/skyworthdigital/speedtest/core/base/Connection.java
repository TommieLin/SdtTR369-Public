package com.skyworthdigital.speedtest.core.base;
...
public class Connection {
    ...
    private static final String USER_AGENT =
            "Speedtest-Android/1.2.3 (SDK " + Build.VERSION.SDK_INT + "; " + Build.PRODUCT
                    + "; Android " + Build.VERSION.RELEASE + ")",
            LOCALE = Build.VERSION.SDK_INT >= 21 ? Locale.getDefault().toLanguageTag() : null;

    public Connection(String url, int connectTimeout, int soTimeout, int recvBuffer,
            int sendBuffer) {
    ...
    public void POST(String path, boolean keepAlive, String contentType, long contentLength)
            throws Exception {
        try {
            if (!path.startsWith("/")) path = "/" + path;
            PrintStream ps = getPrintStream();
    ...
    public HashMap<String, String> parseResponseHeaders() throws Exception {
        try {
            ...
            if (!s.contains("200 OK")) {
                throw new Exception(
                        "Did not receive an HTTP 200 (" + s.trim() + ")");
            }
            while (true) {
                s = readLineUnbuffered();
                if (s.trim().isEmpty()) break;
                if (s.contains(":")) {
                    ret.put(s.substring(0, s.indexOf(":")).trim().toLowerCase(),

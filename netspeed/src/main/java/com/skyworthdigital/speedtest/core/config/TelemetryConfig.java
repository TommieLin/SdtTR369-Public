...
public class TelemetryConfig {
    ...
    public static final String LEVEL_DISABLED = "disabled", LEVEL_BASIC = "basic", LEVEL_FULL =
            "full";

    private void check() {
        if (!(telemetryLevel.equals(LEVEL_DISABLED) || telemetryLevel.equals(LEVEL_BASIC)
                || telemetryLevel.equals(LEVEL_FULL))) {
            throw new IllegalArgumentException(
                    "Telemetry level must be disabled, basic or full");
        }
    }
